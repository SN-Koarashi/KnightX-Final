...
########################################################################
#                          ENEMY INFORMATION                           #
# If you attack it, it will follow until it death                      #
########################################################################
...
func _process(delta):
	...
	if times >= 3:
		...
		Globals.list_enemy3.erase(self)
		queue_free()
...
func _physics_process(delta: float) -> void:
	...
	if !onHit:
		...
		vel = move_and_slide(dir * speed * 1.925)
	
	if vel.y > -75 and vel.y < 75:
		if vel.x > 0:
			$enemy3.set_animation("right")
		elif vel.x < 0:
			$enemy3.set_animation("left")
	else:
		if vel.y > 0:
			$enemy3.set_animation("bottom")
		elif vel.y < 0:
			$enemy3.set_animation("top")
